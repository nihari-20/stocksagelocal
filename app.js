...
function renderStockPage(symbol, insight, history, newsData) {
    ...
    if (newsGrid && newsData) {
        ...
        if (newsData.length === 0) {
            ...
            newsData.slice(0, 6).forEach(item => { // Show top 6
                const div = document.createElement('div');
                div.className = 'news-item';
                div.style.cursor = 'pointer';
                div.onclick = () => window.open(item.url, '_blank');

                const sentimentColor = item.sentiment === 'Positive' ? 'var(--accent-primary)' : (item.sentiment === 'Negative' ? 'var(--accent-error)' : 'var(--text-secondary)');

                div.innerHTML = `
                    <div class="news-content">
                        <div class="news-title" style="color: var(--text-primary);">
                            ${item.headline}
                        </div>
                        <div class="news-meta" style="color: ${sentimentColor}">
